...
export async function POST(request: Request) {
  try {
    ...
    const normalizedRoom = typeof roomCode === 'string' && roomCode.trim()
    ...
    const match = parseRedisJson<{ roundId: string; players: string[] }>(matchRaw as string | null);
    ...
    if (removedCount > 0) {
      return NextResponse.json({ success: true, message: 'Successfully left the queue', refundTx });
    } else {
      return NextResponse.json({ success: true, message: 'Player was not in the queue (already cleaned up)', refundTx });
    }
  } catch (error: any) {
