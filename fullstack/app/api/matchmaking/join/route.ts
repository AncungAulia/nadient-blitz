...
export async function tryMatch(
  ...
  mode: number,
): Promise<{ matched: true; roundId: string; players: string[] } | { matched: false; queueCount: number }> {
  ...
  try {
    ...
    pipeline.set(`round:${roundId}:players`, JSON.stringify(matchedPlayers), { ex: 3600 });
    pipeline.set(
      `round:${roundId}:meta`,
      JSON.stringify({ casual: false, stakeAmount: 10, mode, source: 'public' }),
      { ex: 3600 },
    );
    await pipeline.exec();
    ...
    try {
      const playersData = matchedPlayers.map((a) => ({ wallet_address: a.toLowerCase() }));
      await supabaseAdmin.from('players').upsert(playersData, { onConflict: 'wallet_address' });

      const { data: round } = await supabaseAdmin
        .from('rounds')
        .insert({ game_round_id: roundId.toLowerCase(), mode, source: 'public', status: 'matched' })
        .select('id')
        .single();

      if (round) {
        await supabaseAdmin.from('round_participants').insert(
          matchedPlayers.map((a) => ({ round_id: round.id, wallet_address: a.toLowerCase() })),
        );
      }
    } catch (e) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!walletAddress || typeof mode !== 'number') {
      return NextResponse.json({ error: 'Missing walletAddress or mode' }, { status: 400 });
    }
    ...
    const queueKey = normalizedRoom ? `queue:room:${normalizedRoom}` : `queue:mode:${mode}`;
    const requiredPlayers = getRequiredPlayers(mode);

    await redis.sadd(queueKey, walletAddress);
    await redis.set(
      `queue:player:${walletAddress}`,
      JSON.stringify({ walletAddress, mode, roomCode: normalizedRoom, joinedAt: Date.now() }),
      { ex: QUEUE_TTL },
    );
