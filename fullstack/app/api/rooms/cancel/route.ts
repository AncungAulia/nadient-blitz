...
export async function POST(req: Request) {
  try {
    ...
    if (!isLeader(room, walletAddress)) {
      return NextResponse.json({ error: "Only the leader can cancel the room" }, { status: 403 });
    }

    let refundTx: string | null = null;
    if (room.paid && room.roundId && await roundHasOnChainStakes(room.roundId, room.players)) {
      try {
