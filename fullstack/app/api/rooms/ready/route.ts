...
import { readStake } from "@/lib/refund";
...
export async function POST(req: Request) {
  try {
    ...
    if (!player) return NextResponse.json({ error: "You are not in this room" }, { status: 404 });
    ...
    return NextResponse.json({
      error: "Stake not detected on-chain yet. Wait for transaction confirmation or deposit your stake first.",
      staked: false,
    }, { status: 402 });
  } catch (err: unknown) {
