...
export async function POST(req: Request) {
  try {
    ...
    if (!walletAddress || !target || !codeRaw) {
      return NextResponse.json({ error: "Missing walletAddress/target/code" }, { status: 400 });
    }
    ...
    if (target.toLowerCase() === walletAddress.toLowerCase()) {
      return NextResponse.json({ error: "The leader cannot kick themselves" }, { status: 400 });
    }

    const victim = findPlayer(room, target);
    if (!victim) return NextResponse.json({ error: "Player is not in the room" }, { status: 404 });
