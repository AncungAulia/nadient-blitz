...
import {
  Room, generateRoomCode, generateRoundId, getRoom, saveRoom,
  bindPlayerToRoom, getPlayerRoomCode, modeForPlayerCount, sanitizeRoomName, unbindPlayer,
} from "@/lib/rooms";
...
export async function POST(req: Request) {
  try {
    ...
    const { walletAddress, name, maxPlayers, paid, stakeAmount, difficulty } = body ?? {};
    ...
    if (!Number.isInteger(max) || max < 2 || max > 5) {
      return NextResponse.json({ error: "maxPlayers must be between 2 and 5" }, { status: 400 });
    }
    const stake = Number(stakeAmount ?? 0);
    if (paid && !ALLOWED_STAKES.includes(stake)) {
      return NextResponse.json({ error: "stakeAmount must be 5, 10, 15, or 20" }, { status: 400 });
    }
    if (!ALLOWED_DIFFICULTIES.includes(String(difficulty ?? "medium"))) {
      return NextResponse.json({ error: "difficulty must be easy, medium, hard, or god" }, { status: 400 });
    }
    ...
    if (existing) {
      ...
      if (prev && prev.status === "lobby") {
        return NextResponse.json({ error: "You are still in another room", code: existing }, { status: 409 });
      }
    ...
    if (!code) {
      return NextResponse.json({ error: "Failed to generate a room code. Please try again." }, { status: 500 });
    }
