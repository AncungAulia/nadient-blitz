...
import { accuracyFromDeltaE, deltaE, tier, HSL, normalizeHsl } from "@/src/utils/color";
import { redis, parseRedisJson } from "@/lib/redis";
import { signAndResolve, toTierEnum, PlayerScore } from "@/lib/resolve";
import { supabaseAdmin } from "@/lib/supabase";

function soloRewardAmount(tierName: string): { reward: bigint; tierEnum: number } {
  switch (tierName) {
    case "JACKPOT": return { reward: BigInt(10_000_000), tierEnum: 4 };
    case "GREAT":   return { reward: BigInt(7_500_000),  tierEnum: 3 };
    case "GOOD":    return { reward: BigInt(6_000_000),  tierEnum: 2 };
    default:        return { reward: BigInt(0),           tierEnum: 0 };
  }
}
...
async function persistResolvedRound(params: {
  ...
  const { gameRoundId, mode, source, players, winnerAddress, rewardByAddress, resolved } = params;
...
export async function POST(req: Request) {
  ...
  const timeSec       = typeof rawTime === "number" && isFinite(rawTime) && rawTime >= 0
    ? Math.min(rawTime, 60)
    : undefined;
  ...
  const baseResult = { method: "deltaE", accuracy: acc, deltaE: dE, tier: t.name, payout: t.payout };
  ...
  if (mode === "solo") {
    try {
      ...
      const { txHash, resolved } = await signAndResolve(
        ...
        [reward], [tierEnum], [BigInt(Math.round(acc * 100))],
        BigInt(0), BigInt(5_000_000), true,
      );
  ...
  console.log(`[submit] Player ${addrLower} submitting for round ${roundId}, mode=${mode}, acc=${acc.toFixed(1)}%`);
  ...
  const players = parseRedisJson<string[]>(await redis.get(`round:${roundId}:players`));
  if (!players) {
    console.warn(`[submit] round:${roundId}:players NOT FOUND in Redis`);
    return NextResponse.json({ ...baseResult, waiting: true, message: "Round not found" });
  }
  ...
  const scoreResults = await Promise.all(players.map(p => redis.get(`round:${roundId}:score:${p.toLowerCase()}`)));
  const submittedCount = scoreResults.filter(Boolean).length;

  console.log(`[submit] Round ${roundId}: submitted=${submittedCount}/${required}, players=${JSON.stringify(players)}`);

  if (submittedCount < required) {
    return NextResponse.json({ ...baseResult, waiting: true, submitted: submittedCount, required });
  }

  return resolveMultiplayer(roundId, players, scoreResults as (string | null)[], required, baseResult);
}
...
export async function resolveMultiplayer(
  ...
  if (existingResult) {
    return NextResponse.json({ ...baseResult, resolved: true, ...existingResult });
  }
  ...
  if (!locked) {
    ...
    if (eventualResult) {
      return NextResponse.json({ ...baseResult, resolved: true, ...eventualResult });
    }
  ...
  const stakePerPlayer = BigInt((meta?.stakeAmount ?? 10) as number) * BigInt(1_000_000);
  ...
  if (casual) {
    ...
    return NextResponse.json({ ...baseResult, ...resultData, resolved: true, casual: true });
  }
  ...
  try {
    const { txHash, resolved } = await signAndResolve(
      ...
      [winnerReward], [toTierEnum(winner.tier)], [BigInt(winner.score)],
      devRake, soloRake, false,
    );
