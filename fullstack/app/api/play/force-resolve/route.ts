...
import { deltaE, accuracyFromDeltaE, tier, targetFromRoundId, type TargetDifficulty } from "@/src/utils/color";
...
export async function POST(request: Request) {
  try {
    ...
    const existing = parseRedisJson<object>(await redis.get(`round:${roundId}:result`));
    if (existing) {
    ...
    const meta = parseRedisJson<{ difficulty?: TargetDifficulty }>(await redis.get(`round:${roundId}:meta`));
    ...
    return resolveMultiplayer(roundId, players, scoreResults as (string | null)[], players.length, baseResult);
  } catch (error: any) {
