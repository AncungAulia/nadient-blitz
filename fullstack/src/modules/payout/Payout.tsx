      ...
      <Card className="mb-6 bg-main/10">
        ...
        <CardContent>
          <p className="text-sm text-foreground/60 mb-4">
            Win games to earn <UsdcIcon className="mx-1" />. Withdraw your payout anytime.
          </p>
          <Button className="w-full gap-2 text-lg" size="lg" onClick={claim} disabled={claiming || claimable <= 0}>
            {claiming ? (
              ...
              <>
                ...
                {claimable > 0 ? (
                  <span className="inline-flex items-center gap-2">Claim {formatUsdc(claimable)} <UsdcIcon /></span>
                ) : (
                  "Nothing to Claim"
                )}
              </>
      ...
      <div className="grid grid-cols-1 gap-4">
        <Card className="bg-chart-3/30">
          <CardContent className="pt-3">
            <p className="text-xs font-heading text-foreground/60">YOUR WALLET BALANCE</p>
